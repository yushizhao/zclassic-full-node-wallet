...
public class StartupProgressDialog extends JFrame {
    ...
    public void waitForStartup() throws IOException,
        ...
        Log.info("Splash: checking if bczd is already running...");
        ...
        if (!shouldStartZCashd) {
        	Log.info("Splash: bczd already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	Log.info("Splash: bczd will be started...");
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
            	Log.info("Stopping bczd because we started it - now it is alive: " + 
                ...
                try 
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	Log.info("Waiting for " + ((end - start) / 1000) + " seconds for bczd to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                	Log.info("bczd is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                        //System.out.println("bczd is still alive, killing forcefully");
	                        //daemonProcess.destroyForcibly();
	                    } else
	                    	Log.info("bczd shut down successfully");
                } catch (Exception bad) {
                	Log.error("Couldn't stop bczd!", bad);
                }
            }
        });
